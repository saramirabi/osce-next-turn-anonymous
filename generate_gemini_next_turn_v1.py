#!/usr/bin/env python3
"""Official Gemini full 1500-sample generation (NEXT_TURN_V1)."""
...
EXPERIMENT_MODEL_ID = "gemini-2.5-flash"
...
OUTPUT_CSV = PROJECT_DIR / "final_Gemini_1500_NEXT_TURN_V1.csv"
RAW_JSONL = PROJECT_DIR / "final_Gemini_1500_NEXT_TURN_V1_raw.jsonl"
REPORT_MD = PROJECT_DIR / "final_Gemini_1500_NEXT_TURN_V1_report.md"
CONFIG_JSON = PROJECT_DIR / "Gemini_1500_NEXT_TURN_V1_config.json"
...
def extract_model_id(response: Any) -> str:
    return str(getattr(response, "model_version", None) or EXPERIMENT_MODEL_ID)
...
def is_transient_error(exc: Exception) -> bool:
    ...
    transient_names = {"ResourceExhausted", "ServiceUnavailable", "DeadlineExceeded", "InternalServerError"}
...
def call_model_with_retries(
    ...
    while retry_count <= max_retries:
        try:
            ...
            if is_transient_error(exc) and retry_count < max_retries:
                last_error = str(exc)
...
def get_api_key() -> str:
    for name in ("GEMINI_API_KEY", "GOOGLE_API_KEY"):
        ...
        if value:
            return value
    raise RuntimeError("Set GEMINI_API_KEY or GOOGLE_API_KEY before using --run.")


def verify_model_access(client: genai.Client, parameter_notes: list[str]) -> str:
    ...
    model_id = extract_model_id(response)
    print(f"Experiment model ID: {model_id}")
    return model_id
...
def run_generation(rows: list[dict[str, str]], model_id: str, parameter_notes: list[str]) -> None:
    ...
    for index, row in enumerate(pending, start=1):
        ...
        if response is not None:
            ...
            raw_record["token_usage"] = response_to_dict(getattr(response, "usage_metadata", {}))
...
def write_report(
    ...
    lines = [
        "# Gemini Official Full 1500 Report (NEXT_TURN_V1)",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Official Gemini full 1500-sample generation using NEXT_TURN_V1."
    )
    parser.add_argument(
        "--run",
        action="store_true",
        help="Execute Gemini API calls after local validation.",
    )
