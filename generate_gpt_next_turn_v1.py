#!/usr/bin/env python3
"""Official GPT full 1500-sample generation (NEXT_TURN_V1)."""
...
EXPERIMENT_MODEL_ID = "gpt-5.6-sol"
...
OUTPUT_CSV = PROJECT_DIR / "final_GPT_1500_NEXT_TURN_V1.csv"
RAW_JSONL = PROJECT_DIR / "final_GPT_1500_NEXT_TURN_V1_raw.jsonl"
REPORT_MD = PROJECT_DIR / "final_GPT_1500_NEXT_TURN_V1_report.md"
CONFIG_JSON = PROJECT_DIR / "GPT_1500_NEXT_TURN_V1_config.json"
...
def load_sample_rows(path: Path) -> list[dict[str, str]]:
    ...
    if len(rows) != SAMPLE_SIZE:
        raise ValueError(f"Expected {SAMPLE_SIZE} rows, found {len(rows)}.")
    ...
    if len(set(context_ids)) != SAMPLE_SIZE:
        raise ValueError(f"Sample file does not contain {SAMPLE_SIZE} unique context IDs.")
    if len(set(target_ids)) != SAMPLE_SIZE:
        raise ValueError(f"Sample file does not contain {SAMPLE_SIZE} unique target-turn IDs.")
...
def validate_prompts(rows: list[dict[str, str]]) -> None:
    for row in rows:
        ...
        full_prompt = build_full_prompt_preview(row["context"])
        combined = f"{INSTRUCTIONS}\n\n{user_input}"

        if row["context"] not in user_input:
            raise RuntimeError(f"API input missing context for {row['context_id']}")
        if row["context"] not in full_prompt:
            raise RuntimeError(f"Full prompt missing context for {row['context_id']}")
        ...
        if row["human_doctor_target"] and row["human_doctor_target"] in combined:
            # Allowed only when identical text already appears inside conversation history.
...
def save_config(model_id: str, parameter_notes: list[str] | None = None) -> None:
    config = {
        "model_id": model_id,
...
def extract_reasoning_setting(response: Any, requested: str) -> str:
...
def build_request_kwargs(user_input: str, parameter_notes: list[str]) -> dict[str, Any]:
    kwargs: dict[str, Any] = {
...
def call_model_with_retries(
    ...
    request_kwargs = build_request_kwargs(user_input, parameter_notes)

    while retry_count <= max_retries:
        try:
            response = client.responses.create(**request_kwargs)
            metadata = {
                "parameter_notes": list(parameter_notes),
                "request_kwargs_keys": sorted(request_kwargs.keys()),
            }
            return response, retry_count, None, metadata
        except TypeError as exc:
            message = str(exc)
            handled = False
            if "temperature" in message and "temperature" in request_kwargs:
                parameter_notes.append(
                    f"temperature={request_kwargs['temperature']} unsupported; using API default"
                )
                request_kwargs.pop("temperature", None)
                handled = True
            if "reasoning" in message and "reasoning" in request_kwargs:
                parameter_notes.append(
                    f"reasoning={request_kwargs['reasoning']} unsupported; using API default"
                )
...
def verify_model_access(client: OpenAI, parameter_notes: list[str]) -> str:
    probe_input = build_user_input("Patient: Hello.\n\nDoctor: Hi, what brings you in today?\n\nPatient: Headache.")
    response, _, error, _ = call_model_with_retries(client, probe_input, parameter_notes, max_retries=2)
    if response is None:
        raise RuntimeError(
...
def run_generation(rows: list[dict[str, str]], model_id: str, parameter_notes: list[str]) -> None:
    ...
    for index, row in enumerate(pending, start=1):
        ...
        output_row: dict[str, Any] = {
            ...
            "actual_temperature_if_known": "",
            "reasoning_setting": REQUESTED_REASONING_EFFORT,
            "max_output_tokens": MAX_OUTPUT_TOKENS,
            "generated_doctor_turn": "",
            "input_tokens": "",
            "output_tokens": "",
        ...
        if response is not None:
            ...
            output_row["actual_temperature_if_known"] = (
                "" if response.temperature is None else str(response.temperature)
            )
            output_row["reasoning_setting"] = extract_reasoning_setting(
                response, REQUESTED_REASONING_EFFORT
            )
            if usage is not None:
                output_row["input_tokens"] = usage.input_tokens
                output_row["output_tokens"] = usage.output_tokens
            ...
            raw_record["token_usage"] = (
                usage.model_dump(mode="json") if hasattr(usage, "model_dump") else {}
            )
    ...
    write_report(all_rows, model_id, parameter_notes, total_retries)
...
def write_report(
    ...
    lines = [
        "# GPT Official Full 1500 Report (NEXT_TURN_V1)",
        "",
        ...
        f"- Reasoning setting: `{REQUESTED_REASONING_EFFORT}`",
...
def final_validation(
    sample_rows: list[dict[str, str]], output_rows: list[dict[str, Any]]
) -> None:
    sample_context_ids = {row["context_id"] for row in sample_rows}
    ...
    checks = {
        ...
        "all_contexts_from_sample": set(generated_context_ids) == sample_context_ids,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Official GPT full 1500-sample generation using NEXT_TURN_V1."
    )
    parser.add_argument(
        "--run",
        action="store_true",
        help="Execute paid OpenAI API calls after local validation.",
    )
    ...
    parameter_notes: list[str] = []
    save_config(EXPERIMENT_MODEL_ID, parameter_notes)
    ...
    save_config(model_id, parameter_notes)
    run_generation(rows, model_id, parameter_notes)
