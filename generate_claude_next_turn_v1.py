#!/usr/bin/env python3
"""Official Claude full 1500-sample generation (NEXT_TURN_V1)."""
...
EXPERIMENT_MODEL_ID = "claude-sonnet-4-6"
...
RAW_JSONL = PROJECT_DIR / "final_Claude_1500_NEXT_TURN_V1_raw.jsonl"
REPORT_MD = PROJECT_DIR / "final_Claude_1500_NEXT_TURN_V1_report.md"
CONFIG_JSON = PROJECT_DIR / "Claude_1500_NEXT_TURN_V1_config.json"
...
def extract_generated_text(response: anthropic.types.Message) -> str:
...
def call_model_with_retries(
    ...
    max_retries: int = 6,
) -> tuple[anthropic.types.Message | None, int, str | None, dict[str, Any]]:
    retry_count = 0
...
def verify_model_access(client: anthropic.Anthropic, parameter_notes: list[str]) -> str:
    ...
    print(f"Experiment model ID: {response.model}")
...
def run_generation(rows: list[dict[str, str]], model_id: str, parameter_notes: list[str]) -> None:
    ...
    for index, row in enumerate(pending, start=1):
        ...
        if response is not None:
            ...
            output_row["model_id"] = response.model
            output_row["input_tokens"] = usage.input_tokens
            output_row["output_tokens"] = usage.output_tokens
            output_row["total_tokens"] = usage.input_tokens + usage.output_tokens
...
def write_report(
    ...
    lines = [
        "# Claude Official Full 1500 Report (NEXT_TURN_V1)",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Official Claude full 1500-sample generation using NEXT_TURN_V1."
    )
    parser.add_argument(
        "--run",
        action="store_true",
        help="Execute Anthropic API calls after local validation.",
    )
